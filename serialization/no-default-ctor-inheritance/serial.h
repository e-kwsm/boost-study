...
template<class Archive> void Base::SaveConstDataCommon(Archive &ar, unsigned version) const
{
    std::cerr << __PRETTY_FUNCTION__ << std::endl;
    ar << BOOST_SERIALIZATION_NVP(n);
}

template<class Archive> void Base::serialize(Archive &ar, const unsigned version)
{
    // do nothing
    std::cerr << __PRETTY_FUNCTION__ << std::endl;
}

template<class Archive> void Derived1::serialize(Archive &ar, const unsigned version)
{
    std::cerr << __PRETTY_FUNCTION__ << std::endl;
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(Base);
}

template<class Archive> void Derived2::serialize(Archive &ar, const unsigned version)
{
    std::cerr << __PRETTY_FUNCTION__ << std::endl;
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(Base);
}
